...
import './AccessoriesBookingList.css';
...
const AccessoriesBookingList: React.FC = () => {
    ...
    const fetchBookings = async () => {
        try {
            const response = await fetch('http://localhost:8080/accessoriesbooking/all'); // Ensure the URL is correct
    ...
    const handleDelete = async (id: number) => {
        try {
            const response = await fetch(`http://localhost:8080/accessoriesbooking/all/${id}`, {
                method: 'DELETE',
            });
